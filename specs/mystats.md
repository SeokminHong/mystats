...
- 고정폭은 값이 잘리지 않는 범위 안에서 compact하게 유지한다. 기준 폭은 chart on 상태에서 CPU 92pt, GPU 86pt, Temperature 86pt, Network 108pt, Disk 128pt로 둔다.
...
- Network 메뉴바 항목은 leading icon이 없고 값이 두 줄로 압축되므로 end 여백이 두드러져 보이지 않도록 chart-on 고정폭을 필요한 만큼만 잡는다. 기준 폭은 chart on 108pt, chart off 56pt, chart 48pt로 둔다.
